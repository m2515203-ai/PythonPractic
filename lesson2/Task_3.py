...
even_elements = [c[i] for i in range(len(c)) if i % 2 == 0]
sr_ar = sum(even_elements) / len(even_elements) if even_elements else 0
# Среднее геометрическое элементов списка 'c', расположенных на нечетных местах
from math import prod
odd_elements = [c[i] for i in range(len(c)) if i % 2 == 1]
sr_geom = (prod(odd_elements)) ** (1/len(odd_elements)) if odd_elements else 0
...
print("Ср. арифм. четных элементов:", sr_ar)
print("Ср. геометр. нечетных элементов:", sr_geom)
print("Максимум:", c_max)
print("Минимум:", c_min)
...
# Ср. арифм. четных элементов: 28.875
# Ср. геометр. нечетных элементов: 12.64424088623913
# Максимум: 200
# Минимум: 1.3